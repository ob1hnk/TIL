...
| `tildsa` + Tab | 코테 항목 (문제명 + 레벨·정답률 + 자료구조/유형 태그 + 복잡도 + 코드 블록) |
...
`tildsa`의 `O(?)`와 `정답률 00%`는 일부러 눈에 띄게 둔 것이라 안 채우면 바로 보인다.

`tildsa`는 드롭다운이 세 번 뜬다 — **레벨**(`0`~`5`), **자료구조**(`heap` `deque` `graph` …),
**유형**(`dp` `bfs` `greedy` …). 자료구조와 유형 순서로 태그를 붙여두면 나중에
`--tag heap`으로 "힙으로 푼 문제"만 뽑을 수 있다. 목록에 없는 태그를 쓰거나 자료구조가
딱히 없는 문제면, 스니펫을 다 채우고 나서 그 자리를 직접 고친다.
